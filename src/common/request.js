...
const webtype = true//true为线上,fasle为线下环境
export let tlkpurl = webtype ? 'http://192.168.31.242:8800' : 'http://192.168.31.218:8800' //tlkp
export let header = {
...
async function checkStatus(responses) {
    ...
    if(responses.method == 'DELETE'){
        response = await apiDeleteRequest(responses) //将传入的数据传给封装的接口，拿到值
    }
    let falseCode = [400001, 400002, 400003, 400004, 470000, 470027,401003];
    if (falseCode.includes(response.data.code)) {
    ...
    // console.log(encrypt,timestamp)
    // console.log(header.encrypt,header.timestamp)
    if (encrypt === 'true' && timestamp != "") { //获取到的数据encrypt 是 true的话说明是加密过的并且时间戳不为空
       ...
       if (falseCode.includes(response.status)) { //如果在对应的数组里，则强制跳转登录页，其中401003代表没通行证，跳转登录页
            window.location.href='/login'
        } 
...
async function apiGetRequest(responses) {
...
async function apiPostRequest(responses){
    return new Promise((resolve, reject) => {
        axios({
            method:'post',
            url:responses.url,
...
async function apiDeleteRequest(responses){
    return new Promise((resolve, reject) => {
        axios({
            method:'delete',
            url:responses.url,
